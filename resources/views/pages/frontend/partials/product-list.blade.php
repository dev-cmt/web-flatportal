<!-- resources/views/ecommerce/frontend/partials/product-list.blade.php -->
                        ...
                        <div class="col-xl-4 col-md-6 col-sm-6 infinite-item">
                            <div class="tp-product-item-2 mb-40">
                                <div class="tp-product-thumb-2 p-relative z-index-1 fix w-img">
                                    ...
                                    <div class="tp-product-action-2 tp-product-action-blackStyle">
                                        <div class="tp-product-action-item-2 d-flex flex-column">
                                            ...
                                            <button type="button" class="tp-product-action-btn-2 tp-product-quick-view-btn" data-bs-toggle="modal" data-bs-target="#producQuickViewModal" aria-label="Quick View">
                                            ...
                                            <button type="button" class="add-item tp-product-action-btn-2 tp-product-add-to-wishlist-btn {{ isset($item->wishlist) && $item->wishlist->product_id === $item->id && $item->wishlist->user_id === Auth::id() ? 'bg-dark text-white' : '' }}"
                                                    data-action-name="wishlist"
                                                    data-product-id="{{ $item->id }}"
                                                    data-product-variant-id="{{ count($item->variants) ?? null }}"
                                            ...
                                            <button type="button" class="add-item tp-product-action-btn-2 tp-product-add-to-compare-btn {{ isset($item->compare) && $item->compare->product_id === $item->id && $item->compare->user_id === Auth::id() ? 'bg-dark text-white' : '' }}"
                                                    data-action-name="compare"
                                                    data-product-id="{{ $item->id }}"
                                                    data-product-variant-id="{{ count($item->variants) ?? null }}"
                                ...
                                <div class="tp-product-content-2 pt-15">
                                    ...
                                    <h3 class="tp-product-title-2 truncate-2-lines">
                                        <a href="{{ route('shop-details', ['id' => $item->id, 'url_slug' =>  $item->url_slug]) }}">{{$item->product_name}}</a>
                                    </h3>
                                    ...
                                    <div class="tp-product-price-wrapper-2">
                                        <span class="tp-product-price-2 new-price">{{ round($item->price - ($item->price * $item->discount / 100)) }} ৳</span>
                        ...
                        <div class="col-xl-12">
                            <div class="tp-product-list-item d-md-flex">
                                <div class="tp-product-list-thumb p-relative fix">
                                    <a href="{{ route('shop-details', ['id' => $item->id, 'url_slug' => $item->url_slug]) }}">
                                        <img src="{{asset('public/'. $item->main_image)}}" height="280" alt="">
                                    </a>
                ...
                <nav>
                    <ul>
                            ...
                            <li>
                                <span class="tp-pagination-prev prev page-numbers disabled">
                                    ...
                                    <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M11 1L5 8L11 15" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                                    </svg>
                                </span>
                            </li>
                        @else
                            <li>
                                <a href="{{ $products->previousPageUrl() }}&{{ http_build_query(request()->except('page')) }}" class="tp-pagination-prev prev page-numbers">
                                    <!-- SVG Icon for Previous Page -->
                                    <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M11 1L5 8L11 15" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                                    </svg>
                        ...
                        @php
                            ...
                            $pagesToShow = 5; // Adjust number of pages to show if needed
                        ...
                        @if ($start > 2)
                            <li>
                                <a href="{{ $products->url(1) }}&{{ http_build_query(request()->except('page')) }}">1</a>
                            </li>
                            @if ($start > 3)
                                <li>
                                    <span class="many page">...</span>
                                </li>
                            @endif
                        @elseif ($start == 2)
                            <li>
                        ...
                        @for ($i = $start; $i <= $end; $i++)
                            ...
                            @else
                                <li><a href="{{ $products->url($i) }}&{{ http_build_query(request()->except('page')) }}">{{ $i }}</a></li>
                            @endif
                        @endfor
                        ...
                        @if ($end < $totalPages - 1)
                            ...
                            @endif
                            <li>
                                <a href="{{ $products->url($totalPages) }}&{{ http_build_query(request()->except('page')) }}">{{ $totalPages }}</a>
                            </li>
                        ...
                        @if ($products->hasMorePages())
                            <li>
                                <a href="{{ $products->nextPageUrl() }}&{{ http_build_query(request()->except('page')) }}" class="next page-numbers">
                                    <!-- SVG Icon for Next Page -->
                                    <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M5 1L11 8L5 15" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                                    </svg>
                                </a>
                            </li>
                        @else
